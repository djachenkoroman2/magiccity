...
citygen: tile 1/1 (x=0, y=0) stage 7/9 sampling done in 218.8ms - surface_points=30281, final_points=30281
...
Более полный справочник, включая `tile`, `tiles`, `terrain`, `urban_fields`, `roads`, `buildings`, `parcels`, `fences`, `trees`, `mobile_lidar`, `sampling`, `output` и `worldgen`, находится в `doc/configuration_reference.md`.
...
- `biome_counts`;
- `building_counts`;
- `parcel_counts`, `fence_counts`, `tree_counts`, `parcel_building_alignment`, `building_orientations`, `block_geometry`, `parcel_geometry`;
- `mobile_lidar`, `point_sources`;
- `supported_footprint_types`, `supported_roof_types`, `supported_fence_types`, `supported_tree_crown_shapes`;
- resolved `config`.

Быстрый просмотр:

```bash
jq '{point_count, class_counts, road_models, building_counts, parcel_counts, fence_counts, tree_counts, mobile_lidar, point_sources}' outputs/demo_trees.metadata.json
```
