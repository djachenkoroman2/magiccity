...
- опциональные деревья: biome-aware плотность, natural-ground placement, разные формы крон, trunk/crown classes;
- опциональный транспорт: road/parking/industrial-yard placement, типы `car`, `truck`, `bus`, `emergency`, `tractor`, body/wheel/window classes;
...
| `doc/trees.md` | Деревья: формы крон, плотность по биомам, placement, sampling, LiDAR и metadata. |
| `doc/vehicles.md` | Транспорт: типы, placement modes, sampling, LiDAR и metadata. |
...
- `--verbose` — добавить расширенную статистику по road models, buildings, parcels, fences, trees, vehicles, mobile LiDAR и отдельным sampling-элементам.
...
citygen: tile 1/1 (x=0, y=0) stage 8/10 sampling done in 218.8ms - surface_points=34481, final_points=34481
...
| `configs/demo_trees.yaml` | Деревья: mixed кроны, biome-aware density и классы `tree_trunk`/`tree_crown`. |
| `configs/demo_vehicles.yaml` | Транспорт: road vehicles, parking/yard placement и классы `vehicle_body`/`vehicle_wheel`/`vehicle_window`. |
...
Более полный справочник, включая `tile`, `tiles`, `terrain`, `urban_fields`, `roads`, `buildings`, `parcels`, `fences`, `trees`, `vehicles`, `mobile_lidar`, `sampling`, `output` и `worldgen`, находится в `doc/configuration_reference.md`.
...
- `class_counts`, `class_mapping`, `class_colors`;
...
- `biome_counts`;
- `building_counts`;
- `parcel_counts`, `fence_counts`, `tree_counts`, `vehicle_counts`, `parcel_building_alignment`, `building_orientations`, `block_geometry`, `parcel_geometry`;
- `mobile_lidar`, `point_sources`;
- `supported_footprint_types`, `supported_roof_types`, `supported_fence_types`, `supported_tree_crown_shapes`, `supported_vehicle_types`, `vehicle_catalog`, `vehicle_aliases`;
- resolved `config`.

Быстрый просмотр:

```bash
jq '{point_count, class_counts, road_models, building_counts, parcel_counts, fence_counts, tree_counts, vehicle_counts, mobile_lidar, point_sources}' outputs/demo_vehicles.metadata.json
```
