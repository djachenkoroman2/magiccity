...
Геометрия крыш описана отдельно в `doc/building_roofs.md`.
...
| `weights` | `{}` или стандартная смесь для `mixed` | Веса выбора concrete type при `model: mixed`. |
...
Parcel mode использует прямоугольную MVP-аппроксимацию кварталов и участков поверх текущих road primitives. Это не полноценная GIS-полигонализация дорожного графа, зато каждый принятый building получает `parcel_id`, а metadata показывает агрегаты по blocks/parcels. Подробно этот слой описан в `doc/parcels.md`.
...
- если footprint слишком мал для заданного `courtyard_ratio` и `min_part_width_m`, генератор fallback-ится к `rectangle`.
...
Это удобно проверять после запуска demo configs.
