...
Значения YAML по умолчанию и правила валидации описаны в `doc/configuration_reference.md`. Road primitives, road profiles и hardscape distance подробно описаны в `doc/roads.md`. Ограждения участков включаются отдельной секцией `fences`, но используют геометрию parcels.
...
Если включен слой ограждений, metadata также получает `fence_counts`: количество сегментов, участков с ограждениями, сегментов с фундаментом, воротных разрывов, суммарную длину, среднюю высоту и распределения по типам/сторонам.
